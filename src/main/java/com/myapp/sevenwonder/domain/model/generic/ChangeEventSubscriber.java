...
public class ChangeEventSubscriber {
    ...
    private final Set<Consumer<? super DomainEvent>> observables = new HashSet<>(); //Observables

    public List<DomainEvent> changes() {
        return changes;
    }

    public final ChangeApply changeApply(DomainEvent event) {
        changes.add(event);
    ...
    public final void applyEvent(DomainEvent domainEvent) {
       observables.forEach(consumer -> {
           try {
               ...
               var map = versions.get(domainEvent.type); //Si es null, entonces es la primera vez que ocurre el evento
